...
const T_TabSgName *FindTabSgNameEntry(const char *UserSgName, int VolLetter)   // pointer of SgName and volLetter used to represent convention of International Table
{                                                                              // OK, this function is too hard for me.
#define                   MaxWtdLbl 20                                         // define a constant
...
#define          MaxWtdExt  5
  ...
  if      (VolLetter == 0 || isspace(VolLetter))       // check whether VolLetter equals to 0 or there is space in first element.
    VolLetter = 'A';                                   // If yes, then return the default convention of 'A'
  else if (VolLetter == '1')                           // return 'I' if VolLetter equals to '1'
    VolLetter = 'I';
  else
  {
           VolLetter = toupper(VolLetter);             // obtain the upper value of VolLetter.
    if (   VolLetter != 'I'
        && VolLetter != 'A')
      return NULL;                                     // if non of 'I' or 'A' match with it, then return NULL.
  }

  WtdLbl = WtdLblOriginal;                             // char array

   wl = WtdLbl;
  iwl = 0;

  while (*UserSgName && *UserSgName != ':')                // loop the SgName, make sure there is no ':'         
  {
    if (isspace(*UserSgName) == 0 && *UserSgName != '_')   // if the element is not blank or '_', 
    {
      if (iwl >= MaxWtdLbl)                                // if iwl is larger than max length, return Null
        return NULL;

      *wl++ = *UserSgName;                                 // assign related elements of SgName to wl
      iwl++;
    }
  ...
  *wl = '\0';                                              // assign the last element of wl as '\0'

  if (iwl == 0)
    return NULL;

   we = WtdExt;                                           // char array
  iwe = 0;
  *we = '\0';

  if (*UserSgName)                                        // if there is any element in SgName
  {
    UserSgName++;                                         // move to next element

    while (*UserSgName)                                   // while there are elements in SgName
    {
      if (isspace(*UserSgName) == 0 && *UserSgName != '_')      // if the element is not blank or '_'
      {
        if (iwe >= MaxWtdExt)                                   // if iwe is bigger than MaxWtdExt, return Null
          return NULL;

        *we++ = *UserSgName;                                    // assign other non-blank or '_' elements in SgName to we 
        iwe++;
      }
  ...
  *we = '\0';                                                    // assign the last element of we as blank

  WtdLblOriginChoice = ' ';
  WtdLblBasisChoice  = ' ';

  if (iwl > 1)                                                   // check the value of iwl, if bigger than 1,  
  {
    wl = &WtdLbl[iwl - 1];                                       // ??? do not understand, what's that?
  ...
  if (isalpha(WtdLbl[0]))                                         // isalpha() function check whether chr is english letter
    WtdSgNumber = FindSchoenfliesSymbol(WtdLbl);                  //  if yes, try to find the Schoenflies symbol
  else
  {
    for (wl = WtdLbl; *wl; wl++)
      if (isdigit(*wl) == 0)                                      //  check whether *wl is number, if yes return Null
        return NULL;
...
// enum used to list all possible types of ListOfFiels
...
// function to list information of SgInfo
void ListSgInfo(const T_SgInfo *SgInfo, int F_XYZ, int F_Verbose, FILE *fpout)
{
  ...
  iList = PG_Index(SgInfo->PointGroup);                                   //  returns an index which can be used to access PG_Names and LG_Code_of_PG_Index

  fprintf(fpout, "Point Group  %s\n", PG_Names[iList]);                   // print the point group, PG_Names is a pointer array
  fprintf(fpout, "Laue  Group  %s\n",
    PG_Names[PG_Index(LG_Code_of_PG_Index[iList])]);                      // Laue group 

  fprintf(fpout, "%s\n", XS_Name[SgInfo->XtalSystem]);                    // crystal system, %s used to print a str

  if (SgInfo->UniqueRefAxis != 0 || SgInfo->UniqueDirCode != 0)
  {
    fprintf(fpout, "Unique Axis  ");
    if (SgInfo->UniqueRefAxis != 0 && SgInfo->UniqueRefAxis != 'o')       // unique axis
      fprintf(fpout, "%c", SgInfo->UniqueRefAxis);                        // %c used to print single str
  ...
  if (SgInfo->InversionOffOrigin)
    fprintf(fpout, "Note: Inversion operation off origin\n");             // which means the inversion center is not on origin 

  putc('\n', fpout);

  fprintf(fpout, "Order   %3d\n", SgInfo->OrderL);                        // the order of space group
  fprintf(fpout, "Order P %3d\n", SgInfo->OrderP);                        // the order of point group ???
  putc('\n', fpout);
  ...
  if (F_XYZ || F_Verbose)
  {
    fprintf(fpout, "#List   %3d\n", SgInfo->nList);                        // the list of xyz format symmetry operation
    putc('\n', fpout);

    lsmx = SgInfo->ListSeitzMx;                                            // get the list of seitz matrix
    rmxi = SgInfo->ListRotMxInfo;                                          // get the list of rotation maxtrix information

    if (rmxi == NULL) rmxi = &RotMxInfo;                                   // if nothing in rmxi, 
